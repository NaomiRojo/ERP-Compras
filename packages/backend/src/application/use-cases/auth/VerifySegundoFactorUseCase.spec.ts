...
describe("VerifySegundoFactorUseCase", () => {
  test("verifica el codigo y emite tokens", async () => {
    ...
    const authSessionService: IAuthSessionService = {
      async completeLogin(_usuario, _options) {
  ...
  test("reenvia el desafio 2FA y devuelve un nuevo challenge", async () => {
    const usedChallenges: string[] = [];
    ...
    const authSessionService: IAuthSessionService = {
      ...
      async reissueSecondFactorChallenge(user, challenge) {
        resentToUserId = user.props.id;
        resentChallengeId = challenge.props.id;
        usedChallenges.push(challenge.props.id);
        return {
    ...
    const result = await useCase.resend({
      challengeId: "challenge-1",
    });

    expect(result.challengeId).toBe("challenge-2");
    expect(resentToUserId).toBe(usuario.props.id);
    expect(resentChallengeId).toBe("challenge-1");
    expect(usedChallenges).toEqual(["challenge-1"]);
  });
});
