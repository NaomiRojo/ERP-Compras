...
import { TIPO_DOCUMENTO_IDS } from "src/domain/documentos";
...
const toDetalleDomain = (row: CompraDetalleRow): OrdenCompraDetalle =>
  new OrdenCompraDetalle({
    ...
    baseTipoDocId: row.baseTipoDocId ?? undefined,
    baseEntry: row.baseEntry ?? undefined,
  ...
  public async save(ordenCompra: OrdenCompra): Promise<void> {
    ...
    await this.detailRepository().save(
      ordenCompra.props.detalles.map((detalle) => ({
        ...
        baseTipoDocId: detalle.props.baseTipoDocId ?? null,
        baseEntry: detalle.props.baseEntry ?? null,
        baseLine: detalle.props.baseLine ?? null,
      })),
    );
  }

  public async findById(id: string): Promise<OrdenCompra | null> {
    const header = await this.headerRepository().findOneBy({
      id,
