...
Para cualquier entorno que ya no sea de prueba manual:

- manten `EXPOSE_2FA_CODE=false` para que el backend no devuelva `previewCode`
- guarda credenciales reales en `.env`, `packages/backend/.env.local` o secretos del despliegue, no en archivos de ejemplo
- si un `TWILIO_AUTH_TOKEN` se expone, rotalo en Twilio antes de seguir
